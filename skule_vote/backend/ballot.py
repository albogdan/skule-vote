...
def results(ballots, choices, numSeats):
    ...
    if len(choices) == 2:
        ...
        result["winners"].append(ch1 if (result["rounds"][0][ch1] > result["rounds"][0][ch2]) else ch2)
    ...
    #     one person gets >50% of the vote, keeping track of intermediate rounds for audit reasons    */
        ...
        while stillCounting:
            ...
            if maxVotes >= result["quota"]:
                ...
                result["winners"] = backwardsEliminationProcess(-1, maxVotes, remainingChoices, result["rounds"], currentRound, ballots)
                stillCounting = False

            else:
                backwardsEliminationProcess(minVotes, -1, remainingChoices, result["rounds"], currentRound, ballots)
                currentRound += 1

                # check to make sure there are still valid candidates left
                validCandidates = False
                for i in range(len(remainingChoices)):
                    if remainingChoices[i] != "Eliminated" and remainingChoices[i] != RON:
    ...
    #     Note: Case when RON wins something, stop (any other seats are unfilled) */
        ...
        while stillCounting:
                            ...
                            if ranking[currentRanking] < len(choices): # check for valid ranking
                                ...
                                if name != "Eliminated" and name != "Winner":
                                    ...
                                    if name == "Winner":
                                        ...
                                        voteValue = voteValue * (winnerObject[name] - result["quota"]) / (winnerObject[name])
            ...
            # check for a winner, otherwise keep going and eliminate everyone with the lowest amount of votes total
            if maxVotes >= result["quota"]:
                winnerList = backwardsEliminationProcess(-1, maxVotes, remainingChoices, result["rounds"], currentRound, ballots)
...
def backwardsEliminationProcess(minVotes, maxVotes, candidateList, roundHistory, currentRound, ballots):
    ...
    for i in range(len(candidateList)):
        if candidateList[i] != "Eliminated" and candidateList[i] != "Winner":
            if minVotes == roundHistory[currentRound][candidateList[i]] and candidateList[i] != RON:
