...
class ElectionSessionAdminFormTestCase(SetupMixin, TestCase):
    ...
    def test_1_or_2_csv_files_uploaded_throws_validation_error(self):
        modified_csv_files = self._build_csv_files()
        del modified_csv_files["upload_candidates"]

        form_1 = self._build_form(files=modified_csv_files)  # Two files uploaded
        ...
        form_2 = self._build_form(files=modified_csv_files)  # One file uploaded
    ...
    def test_bad_headers_throws_validation_error(self):
        ...
        modified_csv_files = self._build_csv_files(header=modified_headers)
        form_1 = self._build_form(files=modified_csv_files)
        self.assertFalse(form_1.is_valid())
        self.assertEqual(
        ...
        modified_csv_files = self._build_csv_files(header=modified_headers)
        form_2 = self._build_form(files=modified_csv_files)
        self.assertFalse(form_1.is_valid())
        self.assertEqual(
            "Invalid header discovered. See more info below.",
        ...
        modified_csv_files = self._build_csv_files(header=modified_headers)
        form_3 = self._build_form(files=modified_csv_files)
        self.assertFalse(form_1.is_valid())
    ...
    def test_different_row_lengths_adds_error(self):
        ...
        modified_csv_files = self._build_csv_files(body=modified_body)

        form = self._build_form(files=modified_csv_files)
